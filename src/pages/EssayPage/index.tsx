    ...
    const flashcardsPlugin = (cards: any) => {
      return (tree: any) => {
        ...
        const sortedCards = cards.sort(
          (a: CardData, b: CardData) => a.percent_through - b.percent_through
        );
        ...
        visit(tree, 'paragraph', (node, index, parent) => {
          currentTextLength += node.children.reduce(
            (acc: number, child: any) =>
              acc + (child.value ? child.value.length : 0),
            0
          );
          while (sortedCards.length > 0) {
            const card = sortedCards[0];
            const targetPosition =
              (card.percent_through / 100) * totalTextLength;
      ...
      <div className={style.essayPageContainer}>
        <Header>
          <div className={style.headerContainer}>
            <button onClick={() => navigate('/')} className={style.closeIcon}>
              X
            </button>
          </div>
        </Header>
          ...
          <div className={style.essayContainerContent}>
            {essay ? (
              <>
                ...
                <div className={style.essayContent}>{content}</div>
              </>
            ) : (
              // Handle error state
              <div></div>
