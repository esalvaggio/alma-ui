...
export function useCard(cardData: CardData, onAnswer?: (result: 'correct' | 'incorrect') => void) {
    ...
    const answerCard = useCallback(async (answer: boolean) => {
        try {
    ...
    const handleCardAnswer = useCallback((correct: boolean) => {
        answerCard(correct);
